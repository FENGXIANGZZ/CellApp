...
class Cell_Segmentation(QWidget):
    ...
    def showsegmentbar(self, current, total):
        self.segmentBar.setValue((current + 1) * 100 / total)
...
class CellSegmentThread(QThread):
    ...
    def cell_seg_with_nuc(self):
        ...
        save_path = os.path.join(self.project, "SegStack", self.embryo, self.model, "SegCell", "with_nucleus")
    ...
    def cell_seg_without_nuc(self):
        ...
        save_path = os.path.join(self.project, "SegStack", self.embryo, self.model, "SegCell", "without_nucleus")
        if not os.path.isdir(save_path):
            os.makedirs(save_path)
